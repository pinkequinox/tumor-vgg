...
data_dir = '/media/box/pytorch_classification_data'
dsets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x])
         for x in ['train', 'val']}
dtest = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x])
         for x in ['test']}
dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=6,
                                               shuffle=True, num_workers=4)
                for x in ['train', 'val']}
dtest_loaders = {x: torch.utils.data.DataLoader(dtest[x], batch_size=6,
                                               shuffle=True, num_workers=0)
                for x in ['test']}                
dset_sizes = {x: len(dsets[x]) for x in ['train', 'val']}
dtest_sizes = {x: len(dtest[x]) for x in ['test']}
...
def test_model(model):
    ...
    running_corrects = 0
    for data in dtest_loaders['test']:
	    # get the inputs
	    inputs, labels = data
    ...
    acc = running_corrects / dtest_sizes['test']
...
for popidx in range(30, 22, -1):
    mod_f.pop(popidx)
mod_f.append(nn.MaxPool2d(kernel_size=4, stride=4))
